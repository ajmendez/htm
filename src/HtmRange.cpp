...
void HtmRange::mergeRange(const Key lo, const Key hi)
{
  ...
  //
  if (lo_flag == InclHi) {
    //std::cerr << "Do not Insert " << setw(20) << lo << " into lo" << std::endl;
  } else if (lo_flag == InclLo ||
	   (lo_flag ==  InclOutside)
	   ){
    //std::cerr << "Insert        " << setw(20) << lo << " into lo" << std::endl;
    my_los->insert(lo, 33);
  } 

//   else {
//     std::cerr << "Do not Insert " << setw(20) << lo << " into lo (other)" << std::endl;
//   }

  if (hi_flag == InclLo){
    // std::cerr << "Do not Insert " << setw(20) << hi << " into hi" << std::endl;
    // my_his->insert(lo, 33);
  }
  else if (hi_flag == InclOutside ||
	     hi_flag == InclHi) {
    // std::cerr << "Insert        " << setw(20) << hi << " into hi" << std::endl;
    my_his->insert(hi, 33);
  }
//   else {
//     std::cerr << "Insert        " << setw(20) << hi << " into hi (other) " << std::endl;
//   }
}
...
int HtmRange::stats(int desiredSize)
{
  ...
  if (n_ranges <= desiredSize){
    // cerr << "No work needed, n_ranges is leq desired size: " << n_ranges << " <= " << desiredSize << std::endl;
    return -1;			// Do not do unnecessary work
  }

  keeplooking = 1;
  // cerr << "Start looking with n_ranges = " << n_ranges << std::endl;
