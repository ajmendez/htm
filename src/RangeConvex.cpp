...
#ifdef DIAGNOSE
  for(i = 0; i < corners_.size(); i++) {
    std::cout << corners_[i].ra() << "," << corners_[i].dec() << ":" << corners_[i] << std::endl;
  }
#endif
}
		      ...
		      // const struct SpatialIndex::QuadNode *indexNode)
{
  ...
  std::cout << index_->nameById(index_->idByPoint(v),name)
       << " " << vsum << " " << std::endl;
#endif
...
#ifdef DIAGNOSE
  std::cout << ( mark == pARTIAL ? " partial " :
	    ( mark ==  fULL ? " full " :
	      ( mark == rEJECT ? " reject " :
		" dontknow " ) ) ) << name << std::endl;
    /*
       << v0 << "," << v1 << "," << v2 << " " << std::endl;
       << V(NV(0)) << " , " << V(NV(1)) << " , " << V(NV(2)) << std::endl
