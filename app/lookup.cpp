...
usage(char *name) {
  ...
  std::cout << "usage: " << std::endl
       << name << " [-hex] [-area] [-corner] [-verbose] [-quiet] [-n n] [-text] depth ( x y z | ra dec )" << std::endl;
  std::cout << " [-hex]      : print node id in hexadecimal" << std::endl
       << " [-area]     : print area of node" << std::endl
       << " [-corner]   : print node corners" << std::endl
       << " [-verbose]  : verbose: print all" << std::endl
       << " [-text]     : submit text command (test interface)" << std::endl
       << " [-n n]      : do n lookups (speed test)" << std::endl
       << " [-quiet]    : don't chat, just give back node name" << std::endl
       << " depth       : level of index to return" << std::endl
       << " x y z       : cartesian coordinate of point to look up" << std::endl
       << " ra dec      : J2000 coordinate of point to look up" << std::endl;
  exit(0);
}
...
// ******************************************************
    ...
    if(corner) {
      ...
      std::cout << "Corners :" << std::endl << v1 << std::endl
	   << v2 << std::endl
	   << v3 << std::endl;
      std::cout << "Corners (ra/dec):" << std::endl << v1.ra() << " " << v1.dec() << std::endl
	   << v2.ra() << " " << v2.dec() << std::endl
	   << v3.ra() << " " << v3.dec() << std::endl;
      std::cout << "Differences : " << std::endl
	   << v1.x() - v2.x() << " " << v1.y() - v2.y() << " " << v1.z() - v2.z() << std::endl
	   << v1.x() - v3.x() << " " << v1.y() - v3.y() << " " << v1.z() - v3.z() << std::endl
	   << v3.x() - v2.x() << " " << v3.y() - v2.y() << " " << v3.z() - v2.z() << std::endl;
    }
    if(area) {
      std::cout << "AREA = " << htm->index().area(id) << std::endl;
    }
